...
\begin{equation}
	\begin{split}
		...
		&= \frac{\alpha(q_t)\beta(q_t)}{p(\bm{u})}\\
	\end{split}
\end{equation}

avec \(\alpha(q_t) = p(u_1,\dots,u_t, q_t)\) et \(\beta(q_t) = p(u_{t+1},\dots,u_T \mid q_t)\)

On peut alors aussi exprimer \(p(q_t, q_{t+1} \mid \bm{u})\) en fonction de \(\alpha\) et \(\beta\) :

\begin{equation}
	\begin{split}
		% p(q_t, q_{t+1} \mid \bm{u}) &= p(q_t \mid q_{t+1}, u_1, \dots, u_T)p(q_{t+1} \mid u_1, \dots, u_T)\\
		% &= \frac{p(q_t, q_{t+1}, u_1, \dots, u_T)}{\sum_{q_t}p(q_t, q_{t+1}, u_1, \dots, u_T)}p(q_{t+1} \mid u_1, \dots, u_T)\\
		% &= \frac{p(q_t, u_1, \dots, u_T)p(q_{t+1} \mid q_t)}{\sum_{q_t}p(q_t, u_1, \dots, u_T)p(q_{t+1} \mid q_t)}p(q_{t+1} \mid u_1, \dots, u_T)\\
		% &= \\
		...
		&= \frac{\alpha(q_t) p(u_{t+1} \mid q_{t+1}) \beta(q_{t+1}) A_{q_t, q_{t+1}}}{p(\bm{u})}\\
	\end{split}
\end{equation}
